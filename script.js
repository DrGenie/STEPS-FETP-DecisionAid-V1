...
function getConfigFromForm() {
  ...
  const planningHorizonYears = getPlanningHorizonFromInputs();

      // Enforce coherence between tier duration, planning horizon and number of cohorts
      try {
        const tierMonthsMap = TIER_MONTHS || { frontline: 3, intermediate: 12, advanced: 24 };
        const tierKey = (tier || "frontline").toLowerCase();
        const durationMonths = tierMonthsMap[tierKey] || 12;

        // For horizon logic we allow at most 4 cohorts per year for Frontline (3‑month training, approximated as 3 months per cohort),
        ...
        if (!isFinite(planningHorizonYears) || planningHorizonYears <= 0) {
          planningHorizonYears = minHorizonYears;
        } else if (planningHorizonYears < minHorizonYears) {
          planningHorizonYears = minHorizonYears;
          appState.epiSettings.general.planningHorizonYears = planningHorizonYears;
          const planningInputEl = document.getElementById("planning-horizon");
          if (planningInputEl) {
            planningInputEl.value = String(planningHorizonYears);
          }
          showToast(`Planning horizon adjusted to ${planningHorizonYears} year(s) for the selected tier.`, "warning");
        }

        const maxCohortsLogical = Math.max(1, Math.floor((planningHorizonYears * 12) / monthsPerCohortForHorizon));
        let cohortsAdj = cohorts;
        if (!isFinite(cohortsAdj) || cohortsAdj <= 0) {
          cohortsAdj = 1;
        }
        if (cohortsAdj > maxCohortsLogical) {
          cohortsAdj = maxCohortsLogical;
          const cohortsInputEl = document.getElementById("cohorts");
          if (cohortsInputEl) {
            cohortsInputEl.value = String(cohortsAdj);
          }
          showToast(
            `Number of cohorts capped at ${cohortsAdj} for a ${planningHorizonYears}-year horizon and the selected training duration.`,
            "warning"
          );
        }
        cohorts = cohortsAdj;
      } catch (e) {
        console.warn("Tier/horizon coherence logic failed:", e);
      }
  ...
  return prompt;
      ...
      function copyPromptToClipboard(prompt) {
        if (!navigator.clipboard) {
          ...
          showToast("Clipboard not available. Please copy from the text box.", "warning");
          return;
        }
        navigator.clipboard
          .writeText(prompt)
          .then(() => {
            ...
            showToast("Briefing prompt copied to clipboard.", "success");
          })
          .catch(() => {
            setStatus(false, "Copy failed. Please copy manually.");
            showToast("Copy failed. Please copy manually from the text box.", "error");
          });
      }
      ...
      function handle(target) {
        ...
        if (!appState.savedScenarios || appState.savedScenarios.length === 0) {
          ...
          output.value = "";
          showToast("Save at least one scenario before generating prompts.", "error");
          return;
        }
